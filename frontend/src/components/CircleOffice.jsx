...
export default function CircleOffice() {
  ...
  async function loadOffices(q) {
    ...
    try {
      ...
      const data = await requestJson(`/api/circle-offices/${qs}`);
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.circleofficename.trim()) { setFormError("Circle office name is required."); return; }
    setSaving(true);
    setFormError("");
    try {
      await requestJson("/api/circle-offices/", {
        method: "POST",
        body: JSON.stringify({
          circleofficename: form.circleofficename.trim(),
          officeaddress: form.officeaddress.trim() || null,
          /* slno: form.slno !== "" ? Number(form.slno) : 0, */
        }),
      });
      closeModal();
      loadOffices(searchInput);
  ...
  function openEditModal(office) {
    ...
    setEditForm({
      circleofficename: office.circleofficename || "",
  ...
  async function handleEditSubmit(e) {
    e.preventDefault();
    if (!editForm.circleofficename.trim()) { setEditFormError("Circle office name is required."); return; }
    setEditSaving(true);
    setEditFormError("");
    try {
      await requestJson(`/api/circle-offices/${editingOffice.id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          circleofficename: editForm.circleofficename.trim(),
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      ...
      <section className="border-b border-slate-200 bg-[#f4f6f8]">
        <div className="mx-auto max-w-6xl px-4 pt-3 pb-2 sm:px-6 lg:px-8">
          <h1 className="text-center text-xl font-semibold text-slate-950">Circle Office</h1>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="sticky top-0 z-10 border-b border-slate-500 bg-gray-500">
                  {["#", "Circle Office Name", "Office Address", "Edit"].map((h, i) => (
                    <th key={h} className={`whitespace-nowrap px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-white ${i === 0 || i === 4 ? "text-center" : "text-left"}`}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-5 py-16 text-center text-sm text-slate-400">
                      {searchInput ? `No circle offices found for "${searchInput}".` : "No circle offices added yet."}
                    </td>
                  </tr>
                ) : (
                  paged.map((o, idx) => {
                    ...
                    return (
                      <tr key={o.id} className={`border-b border-slate-100 transition-colors last:border-0 hover:bg-cyan-50/40 ${isEven ? "bg-gray-400" : "bg-white"}`}>
                        ...
                        <td className="h-12 px-5 align-middle font-semibold text-slate-950">{o.circleofficename}</td>
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">
                Edit — <span className="text-cyan-700">{editingOffice.circleofficename}</span>
              </h2>
            ...
            <form onSubmit={handleEditSubmit} className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  <span className={labelCls}>Circle Office Name *</span>
                  <input value={editForm.circleofficename} onChange={(e) => updateEditForm("circleofficename", e.target.value)}
                    className={inputCls} placeholder="e.g. Dhaka Circle Office" required autoFocus />
                </label>
                <label className="block sm:col-span-2">
                  <span className={labelCls}>Office Address</span>
                  <input value={editForm.officeaddress} onChange={(e) => updateEditForm("officeaddress", e.target.value)}
                    className={inputCls} placeholder="Full address" />
                </label>
               {/*  <label className="block">
                  <span className={labelCls}>SL No.</span>
                  <input type="number" min="0" value={editForm.slno} onChange={(e) => updateEditForm("slno", e.target.value)}
                    className={inputCls} placeholder="0" />
                </label> */}
              </div>
              {editFormError && <div className="mt-4 rounded-lg bg-rose-50 px-4 py-3 text-sm text-rose-700">{editFormError}</div>}
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm"
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">Add Circle Office</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  <span className={labelCls}>Circle Office Name *</span>
                  <input value={form.circleofficename} onChange={(e) => updateForm("circleofficename", e.target.value)}
                    className={inputCls} placeholder="e.g. Dhaka Circle Office" required autoFocus />
                </label>
                <label className="block sm:col-span-2">
                  <span className={labelCls}>Office Address</span>
                  <input value={form.officeaddress} onChange={(e) => updateForm("officeaddress", e.target.value)}
                    className={inputCls} placeholder="Full address" />
                </label>
               {/*  <label className="block">
                  <span className={labelCls}>SL No.</span>
                  <input type="number" min="0" value={form.slno} onChange={(e) => updateForm("slno", e.target.value)}
                    className={inputCls} placeholder="0" />
                </label> */}
              </div>
              {formError && <div className="mt-4 rounded-lg bg-rose-50 px-4 py-3 text-sm text-rose-700">{formError}</div>}
              <div className="mt-6 flex justify-end gap-3">
                <button type="button" onClick={closeModal}
                  className="h-10 rounded-lg border border-slate-300 px-4 text-sm text-slate-600 transition hover:bg-slate-50">Cancel</button>
