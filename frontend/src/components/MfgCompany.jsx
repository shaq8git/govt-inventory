...
const emptyForm = { companyname: "", address: "", contactno: "" };

export default function MfgCompany() {
  const perms = getPerms("MFC_COMPANY");
  ...
  async function loadCompanies(q) {
    ...
    try {
      ...
      const data = await requestJson(`/api/mfc-companies/${qs}`);
  ...
  const paged = companies.slice((safePage - 1) * ITEMS_PER_PAGE, safePage * ITEMS_PER_PAGE);
  ...
  async function handleEditSubmit(e) {
    e.preventDefault();
    if (!editForm.companyname.trim()) { setEditFormError("Company name is required."); return; }
    setEditSaving(true);
    setEditFormError("");
    try {
      await requestJson(`/api/mfc-companies/${editingCompany.id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          companyname: editForm.companyname.trim(),
          address: editForm.address.trim() || null,
          contactno: editForm.contactno.trim() || null,
        }),
      });
      closeEditModal();
      loadCompanies(searchInput);
  ...
  async function handleSubmit(e) {
    ...
    setFormError("");
    try {
      await requestJson("/api/mfc-companies/", {
        method: "POST",
        body: JSON.stringify({
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      ...
      <section className="border-b border-slate-200 bg-[#f4f6f8]">
        <div className="mx-auto max-w-6xl px-4 pt-3 pb-2 sm:px-6 lg:px-8">
          <h1 className="text-center text-xl font-semibold text-slate-950">Manufacture Company</h1>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-5 py-16 text-center text-sm text-slate-400">
                      {searchInput
                        ? `No companies found for "${searchInput}".`
                        : "No manufacture companies added yet."}
                    </td>
                  </tr>
                ) : (
                  paged.map((c, idx) => {
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3">
              <p className="text-xs text-slate-600">
                ...
                <span className="font-semibold text-slate-700">
                  {(safePage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(safePage * ITEMS_PER_PAGE, companies.length)}
                </span>{" "}
                of <span className="font-semibold text-slate-700">{companies.length}</span> companies
                {searchInput && (
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">
                Edit — <span className="text-cyan-700">{editingCompany.companyname}</span>
              </h2>
              <button
                type="button"
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <label className="block sm:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => updateEditForm("contactno", e.target.value)}
                    className={inputCls}
      ...
      {modalOpen && (
        ...
        >
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">Add Manufacture Company</h2>
              <button
                type="button"
                onClick={closeModal}
                className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
