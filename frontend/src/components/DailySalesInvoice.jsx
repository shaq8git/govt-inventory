...
function buildPrintHTML(head, items) {
  ...
  const totalAmount = items.reduce((s, r) => s + Number(r.salesprice ?? 0), 0);
...
<head>
  <meta charset="UTF-8"/>
  <title>Sales Invoice ${head.invoiceno}</title>
  <style>
...
<body>
  ...
  <div class="meta">
    ...
    <div><span>Customer: </span><strong>${head.customer_name ?? "—"}</strong></div>
...
function InvoiceModal({ head, onClose }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API}/sales-items/?saleshead=${head.id}`, { headers: authHeaders() })
      .then((r) => r.json())
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div className="grid grid-cols-2 gap-x-10 gap-y-0.5 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-slate-900">{head.customer_name ?? "—"}</p>
            </div>
            <div>
                      ...
                      {items.map((r, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2 text-right tabular-nums text-slate-950">{Number(r.salesrate).toFixed(2)}</td>
                            <td className="px-3 py-2 text-right tabular-nums text-slate-950">{Number(r.salesprice).toFixed(2)}</td>
                          </tr>
                        );
                      })}
...
export default function DailySalesInvoice() {
  const perms = getPerms("DAILY_SALES_INV");
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch(`${API}/sales-heads/?${params}`, { headers: authHeaders() });
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-semibold text-slate-950">Daily Sales Invoice</h1>
          <p className="mt-1 text-sm text-slate-500">Browse sales invoices by date range and view item details</p>
        </div>
      </section>
