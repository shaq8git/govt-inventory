...
export default function ProductList() {
  ...
  async function loadProducts(q) {
    ...
    try {
      ...
      const data = await requestJson(`/api/products/${qs}`);
  ...
  async function loadDropdowns() {
    try {
      const [groups, companies] = await Promise.all([
        requestJson("/api/product-groups/"),
        requestJson("/api/mfc-companies/"),
      ]);
      setProductGroups(normalizeList(groups));
      setMfcCompanies(normalizeList(companies));
    } catch {
  ...
  const paged = products.slice((safePage - 1) * ITEMS_PER_PAGE, safePage * ITEMS_PER_PAGE);
  ...
  async function handleSubmit(e) {
    ...
    if (!form.productgroup) { setFormError("Product group is required."); return; }
    setSaving(true);
    setFormError("");
    try {
      await requestJson("/api/products/", {
        ...
        body: JSON.stringify({
          ...
          mfccompany: form.mfccompany ? Number(form.mfccompany) : null,
  ...
  async function handleEditSubmit(e) {
    e.preventDefault();
    if (!editForm.productname.trim()) { setEditFormError("Product name is required."); return; }
    if (!editForm.productgroup) { setEditFormError("Product group is required."); return; }
    setEditSaving(true);
    setEditFormError("");
    try {
      await requestJson(`/api/products/${editingProduct.id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          productname: editForm.productname.trim(),
          productgroup: Number(editForm.productgroup),
          mfccompany: editForm.mfccompany ? Number(editForm.mfccompany) : null,
          unit: editForm.unit.trim() || null,
          mrp: editForm.mrp === "" ? 0 : Number(editForm.mrp),
        }),
      });
      closeEditModal();
      loadProducts(searchInput);
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      ...
      <section className="border-b border-slate-200 bg-[#f4f6f8]">
        <div className="mx-auto max-w-6xl px-4 pt-3 pb-2 sm:px-6 lg:px-8">
          <h1 className="text-center text-xl font-semibold text-slate-950">Product Information</h1>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="sticky top-0 z-10 border-b border-slate-500 bg-gray-500">
                  {["#", "Prod Code", "Product Name", "Group", "Manufacturer", "MRP", "Edit"].map((h, i) => (
                    <th
                      key={h}
                      className={`whitespace-nowrap px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-white ${
                        [0, 1, 5, 6].includes(i) ? "text-center" : "text-left"
                      }`}
                    >
                      {h}
                    </th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-5 py-16 text-center text-sm text-slate-400">
                      {searchInput ? `No products found for "${searchInput}".` : "No products added yet."}
                    </td>
                  </tr>
                ) : (
                  paged.map((p, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="h-12 px-4 align-middle text-center font-semibold text-cyan-700">
                          {p.prodcode}
                        </td>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3">
              <p className="text-xs text-slate-600">
                ...
                <span className="font-semibold text-slate-700">
                  {(safePage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(safePage * ITEMS_PER_PAGE, products.length)}
                </span>{" "}
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  <span className={labelCls}>Prod Code</span>
                  <input
                ...
                <label className="block">
                  ...
                  <select
                    value={form.mfccompany}
                    onChange={(e) => updateForm("mfccompany", e.target.value)}
                    className={selectCls}
                  >
          ...
          <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">
                Edit — <span className="text-cyan-700">{editingProduct.productname}</span>
              </h2>
              <button
                type="button"
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <label className="block">
                  ...
                  <select
                    ...
                    onChange={(e) => updateEditForm("productgroup", e.target.value)}
                    className={selectCls}
                    required
                  >
                    <option value="">— Select group —</option>
                ...
                <label className="block">
                  <span className={labelCls}>Manufacturer</span>
                  <select
                    value={editForm.mfccompany}
                    onChange={(e) => updateEditForm("mfccompany", e.target.value)}
                    className={selectCls}
                  >
                    <option value="">— None —</option>
                    {mfcCompanies.map((c) => (
                ...
                <label className="block">
                  ...
                  <input
                    ...
                    step="0.01"
                    value={editForm.mrp}
                    onChange={(e) => updateEditForm("mrp", e.target.value)}
                    className={inputCls}
                    placeholder="0.00"
                  />
                </label>
