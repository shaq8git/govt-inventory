...
export default function Damage() {
  ...
  const [dmVouchercodeId, setDmVouchercodeId] = useState(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/voucher-codes/`, { headers: authHeaders() })
      ...
      .then((d) => {
        ...
        const dm = list.find((v) => v.shortname === "DM");
        if (dm) setDmVouchercodeId(dm.id);
      });
  }, []);
  ...
  async function handleSaveRow(tempId) {
    ...
    if (!currentHeadId) {
      ...
      try {
        const res = await fetch(`${API}/damage-heads/`, {
          method: "POST",
          headers: authHeaders(),
          body: JSON.stringify({
            customer: headForm.customer_id,
            vouchercode: dmVouchercodeId || null,
        ...
        setRows((prev) =>
          prev.map((r) =>
            r.tempId === tempId
              ? { ...r, saving: false, error: "Failed to create damage head." }
              : r
          )
        );
        return;
    ...
    try {
      const res = await fetch(`${API}/damage-items/`, {
        ...
        headers: authHeaders(),
        body: JSON.stringify({
          damagehead: currentHeadId,
          product: Number(row.product_id),
          quantity: Number(row.quantity),
          damagerate: Number(row.damagerate) || 0,
        }),
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
  ...
  async function handleDeleteRow(tempId) {
    ...
    try {
      const res = await fetch(`${API}/damage-items/${row.itemId}/`, {
        method: "DELETE",
        headers: authHeaders(),
      });
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-semibold text-slate-950">Damage</h1>
          <p className="mt-1 text-sm text-slate-500">Record damaged items</p>
        </div>
      </section>
