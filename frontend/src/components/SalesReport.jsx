...
function buildPrintHTML(rows, filters) {
  ...
  const totalAmount = rows.reduce((s, r) => s + Number(r.salesprice ?? 0), 0);
  ...
  const bodyRows = rows
    .map(
      ...
      <tr class="${i % 2 === 0 ? "alt" : ""}">
        ...
        <td class="right">${Number(r.salesrate).toFixed(2)}</td>
        <td class="right">${Number(r.salesprice).toFixed(2)}</td>
        <td>${r.remark || ""}</td>
      </tr>`
    )
    .join("");
...
export default function SalesReport() {
  ...
  async function handleGenerate() {
    ...
    if (dateFrom) params.set("saleshead__invoicedate__gte", dateFrom);
    if (dateTo) params.set("saleshead__invoicedate__lte", dateTo);
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-semibold text-slate-950">Sales / Receiver Report</h1>
          <p className="mt-1 text-sm text-slate-500">Filter sales records by product and date</p>
        </div>
      </section>
              ...
              <table className="min-w-full border-separate border-spacing-0 text-sm">
                <thead className="bg-slate-900 text-xs font-semibold uppercase tracking-wide text-slate-300">
                  <tr>
                    ...
                    <th className="min-w-48 border-b border-slate-700 px-3 py-3 text-left">Product</th>
                    <th className="border-b border-slate-700 px-3 py-3 text-left">Customer</th>
                    <th className="w-24 border-b border-slate-700 px-3 py-3 text-center">Qty</th>
                    <th className="w-28 border-b border-slate-700 px-3 py-3 text-right">Rate</th>
                    <th className="w-32 border-b border-slate-700 px-3 py-3 text-right">Amount</th>
                ...
                <tbody>
                  {rows.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="py-8 text-center text-sm text-slate-400">No records found.</td>
                    </tr>
                  ) : (
                    <>
                      {rows.map((row, idx) => {
                        ...
                        return (
                          <tr key={row.id} className={`${isDark ? "bg-gray-400" : "bg-white"} border-b ${isDark ? "border-slate-500" : "border-slate-200"} last:border-0`}>
                            <td className="px-3 py-2 text-center text-xs font-semibold text-slate-950">{idx + 1}</td>
                            <td className="px-3 py-2 font-mono tabular-nums text-slate-950">{row.invoicedate ?? "—"}</td>
                            <td className="px-3 py-2 font-mono text-slate-950">{row.invoiceno ?? "—"}</td>
                            ...
                            <td className="px-3 py-2 text-slate-950">{row.customer_name ?? "—"}</td>
                            <td className="px-3 py-2 text-center tabular-nums text-slate-950">{row.quantity}</td>
                            <td className="px-3 py-2 text-right tabular-nums text-slate-950">{Number(row.salesrate).toFixed(2)}</td>
                            <td className="px-3 py-2 text-right tabular-nums text-slate-950">{Number(row.salesprice).toFixed(2)}</td>
                            <td className="px-3 py-2 text-xs text-slate-950">{row.remark || ""}</td>
                          </tr>
                        );
                      })}
