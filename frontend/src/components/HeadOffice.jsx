...
export default function HeadOffice() {
  ...
  async function loadOffices() {
    setLoading(true);
    try {
      const data = await requestJson("/api/head-offices/");
  ...
  function openEditModal(office) {
    ...
    setEditForm({ officename: office.officename || "", address: office.address || "" });
    setEditFormError("");
  ...
  async function handleEditSubmit(e) {
    ...
    setEditFormError("");
    try {
      await requestJson(`/api/head-offices/${editingOffice.id}/`, {
        method: "PATCH",
        body: JSON.stringify({
  ...
  async function handleSubmit(e) {
    ...
    setFormError("");
    try {
      await requestJson("/api/head-offices/", {
        method: "POST",
        body: JSON.stringify({
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      ...
      <section className="border-b border-slate-200 bg-[#f4f6f8]">
        <div className="mx-auto max-w-6xl px-4 pt-3 pb-2 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h1 className="shrink-0 text-xl font-semibold text-slate-950">Head Office</h1>
            {perms.c && (
              <button
                type="button"
                onClick={openModal}
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="sticky top-0 z-10 border-b border-cyan-700 bg-gradient-to-r from-cyan-600 to-emerald-600">
                  {["#", "Office Name", "Address", "Edit"].map((h, i) => (
                    <th
                      key={h}
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">
                Edit — <span className="text-cyan-700">{editingOffice.officename}</span>
              </h2>
              <button
                type="button"
              ...
              <div className="space-y-4">
                <label className="block">
                  ...
                  <input
                    ...
                    className={inputCls}
                    placeholder="e.g. Directorate Head Office"
                    required
                    autoFocus
                  />
                </label>
      ...
      {modalOpen && (
        ...
        >
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">Add Head Office</h2>
              <button
                type="button"
                onClick={closeModal}
                className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
