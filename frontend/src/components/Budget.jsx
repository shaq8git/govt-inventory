...
export default function Budget() {
  ...
  function handleProductChange(productId) {
    ...
    if (prod) setPrice(prod.purchaserate ?? "");
  }
  ...
  function handleAddRow() {
    ...
    setRows((prev) => [
      ...prev,
      {
        ...
        price: Number(price),
        current_qty: prod.currentqty ?? 0,
        current_rate: prod.purchaserate ?? 0,
      },
    ]);
    setSelectedProduct("");
    setQuantity("");
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setSuccess(`Budget ${data.budgetno} submitted successfully.`);
      setRows([]);
    } catch (e) {
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-5xl px-4 py-5 sm:px-6">
          ...
          <p className="mt-1 text-sm text-slate-500">Submit an annual product budget for approval</p>
        </div>
      </section>
        ...
        <div className="rounded-lg border border-slate-200 bg-white px-6 py-5 shadow-sm">
          ...
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col gap-1">
              ...
              <select
                value={groupFilter}
                onChange={(e) => { setGroupFilter(e.target.value); setSelectedProduct(""); setPrice(""); }}
            ...
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th className="border-b border-slate-200 px-3 py-3 text-right">Price (৳)</th>
                  <th className="border-b border-slate-200 px-3 py-3 text-center">Remove</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.id} className={`${isDark ? "bg-slate-50" : "bg-white"} border-b ${isDark ? "border-slate-200" : "border-slate-200"}`}>
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-slate-950">{Number(r.price).toFixed(2)}</td>
