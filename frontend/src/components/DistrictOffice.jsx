...
const emptyForm = { districtofficename: "", officeaddress: "" };

export default function DistrictOffice() {
  const perms = getPerms("DISTRICT_OFFICE");
  ...
  async function loadOffices(q) {
    ...
    try {
      ...
      const data = await requestJson(`/api/district-offices/${qs}`);
  ...
  function openEditModal(office) {
    ...
    setEditForm({
      districtofficename: office.districtofficename || "",
  ...
  async function handleEditSubmit(e) {
    e.preventDefault();
    if (!editForm.districtofficename.trim()) { setEditFormError("District office name is required."); return; }
    setEditSaving(true);
    setEditFormError("");
    try {
      await requestJson(`/api/district-offices/${editingOffice.id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          districtofficename: editForm.districtofficename.trim(),
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.districtofficename.trim()) { setFormError("District office name is required."); return; }
    setSaving(true);
    setFormError("");
    try {
      await requestJson("/api/district-offices/", {
        method: "POST",
        body: JSON.stringify({
          districtofficename: form.districtofficename.trim(),
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={4} className="px-5 py-16 text-center text-sm text-slate-400">
                      {searchInput
                        ? `No district offices found for "${searchInput}".`
                        : "No district offices added yet."}
                    </td>
                  </tr>
                ) : (
                  paged.map((o, idx) => {
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3">
              <p className="text-xs text-slate-600">
                ...
                <span className="font-semibold text-slate-700">
                  {(safePage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(safePage * ITEMS_PER_PAGE, offices.length)}
                </span>{" "}
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">
                Edit — <span className="text-cyan-700">{editingOffice.districtofficename}</span>
              </h2>
              <button
                type="button"
            ...
            <form onSubmit={handleEditSubmit} className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  <span className={labelCls}>District Office Name *</span>
                  <input
                    value={editForm.districtofficename}
                    onChange={(e) => updateEditForm("districtofficename", e.target.value)}
                    className={inputCls}
                    placeholder="e.g. Dhaka District Office"
                    required
                    autoFocus
                  />
                </label>
      ...
      {modalOpen && (
        ...
        >
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">Add District Office</h2>
              <button
                type="button"
                onClick={closeModal}
                className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  <span className={labelCls}>District Office Name *</span>
                  <input
                    value={form.districtofficename}
                    onChange={(e) => updateForm("districtofficename", e.target.value)}
                    className={inputCls}
                    placeholder="e.g. Dhaka District Office"
                    required
                    autoFocus
