...
export default function PurchaseReturns() {
  ...
  const [pnVouchercodeId, setPnVouchercodeId] = useState(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/voucher-codes/`, { headers: authHeaders() })
      ...
      .then((d) => {
        ...
        const pn = list.find((v) => v.shortname === "PN");
        if (pn) setPnVouchercodeId(pn.id);
      });
  }, []);
  ...
  async function handleSaveRow(tempId) {
    ...
    if (!currentHeadId) {
      ...
      if (!headForm.supplier_id) {
        setRows((prev) =>
          prev.map((r) =>
            r.tempId === tempId
              ? { ...r, saving: false, error: "Select a supplier in the head section." }
              : r
          )
        );
        return;
      ...
      try {
        const res = await fetch(`${API}/purret-heads/`, {
          method: "POST",
          headers: authHeaders(),
          body: JSON.stringify({
            supplier: headForm.supplier_id,
            vouchercode: pnVouchercodeId || null,
    ...
    try {
      const res = await fetch(`${API}/purret-items/`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({
          purrethead: currentHeadId,
  ...
  async function handleDeleteRow(tempId) {
    ...
    try {
      const res = await fetch(`${API}/purret-items/${row.itemId}/`, {
        method: "DELETE",
        headers: authHeaders(),
      });
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-semibold text-slate-950">Purchase Returns</h1>
          <p className="mt-1 text-sm text-slate-500">Record items returned to supplier</p>
        </div>
      </section>
            ...
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th className="w-32 border-b border-slate-200 px-3 py-3 text-center">Pur. Rate</th>
              ...
              <tbody>
                {rows.map((row, idx) => {
                  ...
                  const borderCls = isDark || row.saved ? "border-slate-200" : "border-slate-200";
                  return (
                      ...
                      <td className="px-3 py-2">
                        {row.saved ? (
                          ...
                          <select
                            ...
                            className="h-9 w-full rounded border border-slate-300 bg-white px-2 text-sm text-slate-900 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200"
                          >
                      ...
                      <td className="px-3 py-2">
                        {row.saved ? (
                          ...
                          <input
                            ...
                            className="h-9 w-full rounded border border-slate-300 bg-white px-2 text-center text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200"
                            placeholder="0"
                          />
                        )}
                      </td>
                      <td className="px-3 py-2">
                        {row.saved ? (
                          ...
                          <input
                            readOnly
                            value={row.purrate}
                            className="h-9 w-full rounded border border-slate-200 bg-slate-100 px-2 text-center text-sm text-slate-500 cursor-default"
                            placeholder="0.00"
                          />
                        )}
                      </td>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          {row.saved ? (
                            ...
                            <button
                              ...
                              disabled={row.saving}
                              title="Save row"
                              className="flex h-8 w-8 items-center justify-center rounded border border-cyan-300 bg-cyan-50 text-cyan-700 transition hover:bg-cyan-100 disabled:opacity-50"
                            >
                              {row.saving ? (
