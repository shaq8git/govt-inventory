...
const emptyForm = { supname: "", contact: "", address: "", shipingadr: "" };

export default function Supplier() {
  const perms = getPerms("SUPPLIER");
  ...
  async function loadSuppliers(q) {
    ...
    try {
      ...
      const data = await requestJson(`/api/suppliers/${qs}`);
  ...
  const paged = suppliers.slice((safePage - 1) * ITEMS_PER_PAGE, safePage * ITEMS_PER_PAGE);
  ...
  function openEditModal(supplier) {
    ...
    setEditForm({
      ...
      shipingadr: supplier.shipingadr || "",
    });
    setEditFormError("");
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.supname.trim()) { setFormError("Supplier name is required."); return; }
    setSaving(true);
    setFormError("");
    try {
  ...
  async function handleEditSubmit(e) {
    e.preventDefault();
    if (!editForm.supname.trim()) { setEditFormError("Supplier name is required."); return; }
    setEditSaving(true);
    setEditFormError("");
    try {
      await requestJson(`/api/suppliers/${editingSupplier.id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          supname: editForm.supname.trim(),
    ...
    <main className="min-h-screen bg-[#f4f6f8] text-slate-900">
      ...
      <section className="border-b border-slate-200 bg-[#f4f6f8]">
        <div className="mx-auto max-w-6xl px-4 pt-3 pb-2 sm:px-6 lg:px-8">
          <h1 className="text-center text-xl font-semibold text-slate-950">Supplier</h1>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="sticky top-0 z-10 border-b border-slate-500 bg-gray-500">
                  {["#", "Supplier Name", "Contact", "Address", "Shipping Address", "Edit"].map((h, i) => (
                    <th
                      key={h}
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-5 py-16 text-center text-sm text-slate-400">
                      {searchInput
                        ? `No suppliers found for "${searchInput}".`
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3">
              <p className="text-xs text-slate-600">
                ...
                <span className="font-semibold text-slate-700">
                  {(safePage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(safePage * ITEMS_PER_PAGE, suppliers.length)}
                </span>{" "}
                of <span className="font-semibold text-slate-700">{suppliers.length}</span> suppliers
                {searchInput && (
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  ...
                  <input
                    value={form.supname}
                    onChange={(e) => updateForm("supname", e.target.value)}
                    className={inputCls}
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-semibold text-slate-950">
                Edit — <span className="text-cyan-700">{editingSupplier.supname}</span>
              </h2>
              <button
                type="button"
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => updateEditForm("supname", e.target.value)}
                    className={inputCls}
                    placeholder="e.g. Acme Supplies Ltd."
                    required
                    autoFocus
